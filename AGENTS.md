...
When you've made a change, run the app yourself so the result is visible.
...
**Codex note:** there is no `run_in_background` flag here. To avoid blocking, only launch `scripts/run` in a detached background process with logs redirected to a file.

**CORRECT usage (Codex):**
```bash
# Detach and capture build output
nohup ./scripts/run > /tmp/reader_run.log 2>&1 &
```

**WRONG - these will block forever or lose logs:**
```bash
...
After launching in the background, DO NOT poll the output or wait. The app will launch and you're done. Move on immediately.
